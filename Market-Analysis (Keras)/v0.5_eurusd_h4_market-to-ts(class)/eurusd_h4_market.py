...
#    model.add( normalization.BatchNormalization( batch_input_shape=(None, 1, 15) ) )
    model.add( GRU( 50, input_shape=(1, data_x.shape[2]) ) )#, dropout=0.2, recurrent_dropout=0.2
#    model.add( Reshape( (1,1) ) )
#    model.add( GRU( 4 ) )
    model.add( Dense( 1, activation='tanh' ) )#tanh

model.compile( loss='mse', optimizer='adam', metrics=['mae', 'acc'] )#mse
